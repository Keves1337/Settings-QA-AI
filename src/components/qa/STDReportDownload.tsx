...
import { downloadSTDReport, downloadHebrewSTDReport } from "@/utils/pdfGenerator";
...
export const STDReportDownload = ({ report }: STDReportDownloadProps) => {
  ...
  const [isDownloadingHebrew, setIsDownloadingHebrew] = useState(false);
  ...
  const handleDownloadHebrew = async () => {
    try {
      setIsDownloadingHebrew(true);
      console.log('Starting Hebrew PDF generation with report:', report);
      ...
      console.log('Generating Hebrew PDF with', testResults.length, 'tests');
      await downloadHebrewSTDReport(testResults, metadata);
      
      toast({
        title: "Hebrew Report Downloaded",
        description: `Hebrew STD report with ${testResults.length} tests has been generated`,
      });
    } catch (error) {
      console.error('Error generating Hebrew PDF:', error);
      toast({
        title: "Download Failed",
        description: "Failed to generate Hebrew PDF report. Check console for details.",
        variant: "destructive",
      });
    } finally {
      setIsDownloadingHebrew(false);
    }
  };
  ...
  return (
    <div className="flex gap-2">
      ...
      <Button 
        onClick={handleDownloadHebrew}
        className="gap-2"
        size="lg"
        variant="outline"
        disabled={isDownloadingHebrew}
      >
        <Languages className="w-4 h-4" />
        {isDownloadingHebrew ? 'Generating...' : 'Download Hebrew PDF'}
      </Button>
    </div>
  );
};
